...
notion_category: <카테고리>     # 예: Backend / .NET / Architecture / Automation
...
- **주제문 먼저.** 첫 문단은 굵은 한 문장으로 핵심 주장을 던진다. 예: `**루프는 "잘 도는 것"이 아니라 "멈출 줄 아는 것"이다.**`
- **구체적 수치·사실 우선.** "빨라졌다" 대신 "91초 → 0.04초(약 2000배)". 소스노트에 없는 수치는 지어내지 않는다.
- 짧고 단정적인 한국어. 군더더기·과장·마케팅 표현 배제.
- `##`/`###` 소제목으로 구획. 코드는 펜스(```lang) — 언어 표기 필수(하이라이트·Notion 매핑용).
- 목록은 `-`. 표는 필요할 때만.
- 길이: 800~2000자 본문 권장. 소스가 얇으면 억지로 늘리지 않는다.
- **사실 왜곡 금지.** 소스노트에 있는 내용만 재구성한다. 새 주장/벤치마크/인용을 만들지 않는다. 불확실하면 뺀다.
...
> - 위 House-style 을 지킬 것. 특히 **주제문 먼저**, **구체적 수치는 소스에 있는 것만**, **없는 사실은 만들지 말 것**.
> - 소스의 구조(Summary/Details/Examples/Gotchas 또는 Problem/Options/Decision)를 그대로 베끼지 말고 **읽는 글**로 재서술. 코드 예시는 핵심만 발췌.
> - 출력은 위 frontmatter 규격 + 본문. `notion_slug` 는 ascii-kebab, `notion_category`/`notion_tags` 는 소스의 topic 태그에서 도출.
