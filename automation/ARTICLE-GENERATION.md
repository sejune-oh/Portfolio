...
- **제목은 주제만 간략히.** `notion_title` 은 긴 설명형·주장형 문장이 아니라 글의 주제가 바로 드러나는 짧은 제목으로 쓴다. 같은 주제를 여러 편으로 나누는 시리즈는 `공통 제목 (1)`, `공통 제목 (2)` 형식(공통 제목 + 공백 + `(숫자)`)으로 통일한다.
...
> - `notion_title` 은 주제만 드러나는 간략한 제목. 시리즈면 `공통 제목 (1)`, `공통 제목 (2)` 형식.
