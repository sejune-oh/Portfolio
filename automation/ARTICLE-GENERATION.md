...
- **자연스럽고 읽기 편한 한국어.** 딱딱한 기술 문서체를 지양하고 사람이 설명하듯 부드럽게 잇는다(문장 길이 변주, 자연스러운 전환). 다만 군더더기·과장·마케팅 표현은 배제한다. 기본 어미는 기존 발행 글과 같은 `~한다` 체.
...
스타일 레퍼런스가 작성 규칙보다 우선한다. 샘플에 없는 표현 습관을 크게 벗어나 만들지는 않는다. loop-engineering 글의 `~한다` 체와 1인칭 절제는 유지하되, **톤은 더 자연스럽고 읽기 편하게 — 딱딱하지 않게** 푼다(짧은 단정 문장만 반복하지 말고 설명하듯 잇기). 더 부드러운 말투의 새 샘플이 생기면 그 글을 새 레퍼런스로 삼는다.
